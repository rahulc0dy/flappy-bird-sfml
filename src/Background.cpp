...
Background::Background(AssetManager& assetManager) 
    ...
    m_background2.setPosition(m_background1.getGlobalBounds().width, 0);
    ...
    m_ground2.setPosition(m_ground1.getGlobalBounds().width, GROUND_Y);
}

void Background::update(float deltaTime) {
    // Update background
    m_backgroundOffset += BACKGROUND_SPEED * deltaTime;
    float bgWidth = m_background1.getGlobalBounds().width;
    ...
    float groundWidth = m_ground1.getGlobalBounds().width;
